...
const { createMock } = vi.hoisted(() => ({
  createMock: vi.fn<
    (args: { data: { email: string; passwordHash: string } }) => Promise<{
      id: string;
      email: string;
    }>
  >(),
...
function firstCreateArg(): { data: { email: string; passwordHash: string } } {
...
describe("POST /api/auth/signup", () => {
  ...
  it("creates the account and returns 201 with the selected fields", async () => {
    ...
    const response = await POST(
      postJson({ email: "  Ada@Example.com ", password: PASSWORD }),
    );
  ...
  it("stores a bcrypt hash, never the plaintext password", async () => {
    ...
    await POST(postJson({ email: "ada@example.com", password: PASSWORD }));
  ...
  it("returns 400 on a short password without touching the database", async () => {
    const response = await POST(
      postJson({ email: "ada@example.com", password: "short" }),
    );
  ...
  it("returns 400 on a malformed email without touching the database", async () => {
    const response = await POST(
      postJson({ email: "not-an-email", password: PASSWORD }),
    );
  ...
  it("returns 409 when the unique constraint rejects the insert", async () => {
